...
void StudentTests() {
  ...
  std::cout << std::endl;
  std::cout << "88 Cards" << std::endl;

  Node* deck = CreateDeck(88);
  Node* hands[4];

  CutDeck(deck,top,bottom,"perfect");
  PrintDeckPrimary(" Top: ", top);
  PrintDeckPrimary(" Bot: ", bottom);
  deck = Shuffle(bottom,top,"perfect");
  PrintDeckPrimary("Deck: ", bottom);
  CutDeck(deck,top,bottom,"perfect");
  deck = Shuffle(bottom,top,"perfect");
  PrintDeckPrimary("Deck: ", bottom);
  CutDeck(deck,top,bottom,"perfect");
  ...
  PrintDeckPrimary("Deck: ", bottom);
  Node* sorted = SortHand(deck);
  PrintDeckSorted("Sorted deck", sorted);

  std::cout << std::endl;

  Deal(deck,hands,4,"one-at-a-time",15);
  PrintDeckPrimary("   0: ", hands[0]);
  PrintDeckPrimary("   1: ", hands[1]);
  PrintDeckPrimary("   2: ", hands[2]);
  PrintDeckPrimary("   3: ", hands[3]);
  PrintDeckPrimary("Deck: ", deck);
  ...
  PrintDeckSorted("Sorted   0: ", sorted_hands[0]);
  PrintDeckSorted("Sorted   1: ", sorted_hands[1]);
  PrintDeckSorted("Sorted   2: ", sorted_hands[2]);
  PrintDeckSorted("Sorted   3: ", sorted_hands[3]);
  ...
  std::cout << "Odd number deck(15)" << std::endl;
  

  deck = CreateDeck(15);
  assert (DeckSize(deck) == 15);
  PrintDeckPrimary("Deck: ", deck);
  CutDeck(deck,top,bottom,"perfect");
  PrintDeckPrimary(" Top: ", top);
  PrintDeckPrimary(" Bot: ", bottom);
  ...
  DeleteAllCards(deck);
  std::cout<<std::endl;
  std::cout << "One Card" << std::endl;

  deck = CreateDeck(1);
  std::cout << "Deck size: " << DeckSize(deck) << std::endl;
  PrintDeckPrimary("Deck: ", deck);
  CutDeck(deck,top,bottom,"perfect");
  PrintDeckPrimary(" Top: ", top);
  PrintDeckPrimary(" Bot: ", bottom);
  ...
  std::cout<<"testing copy(if not aborted by assert, then passed)"<<std::endl;
  deck = CreateDeck(52);
  SortHand(deck);
  Node* deck_cnm = CopyDeck(deck);
  assert(SanityCheckBoth(deck_cnm));
  assert(SanityCheckPrimary(deck_cnm));
  Node* sorted_cnm = SortHand(deck_cnm);
  assert(SanityCheckSorted(sorted_cnm));

  DeleteAllCards(deck);
  DeleteAllCards(deck_cnm);

...
public:
  ...
  // Cancel move of robot i with old pos Pos
  void cancelMove(int i, Position beforeMove);
  ...
  // get the positions of robots return as a vector
  std::vector<Position> getRobotPositions() const;
  void restorePositions(std::vector<Position> &old);

  // satisfy all goals
  bool allGoalHold() const;

...
int count(vector<vector<bool>> &path,vector<vector<bool> > const& blocked_grid,int x,int y){
  if(x == 0 && y==0){
    return 1;
  }
  else{
    if(!blocked_grid[x][y]){
      ...
      if(x>0 && y>0){return count(blocked_grid,x-1,y)+count(blocked_grid,x,y-1);}
      if(x>0 && y == 0){ return count(blocked_grid,x-1,y);}
      if(x == 0 && y>0){ return count(blocked_grid,x,y-1);}
    }
    else{
      return 0;
    }
...
int  a;
a = count(blocked_grid,start_x,start_y);
  // Start here with your code...
  std::cout<<"Total possible: "<<a<<std::endl;

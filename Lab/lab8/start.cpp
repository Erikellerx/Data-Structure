...
void print_grid(vector<vector<bool> > const& blocked_grid, vector<Point> &path, int start_x, int start_y) {
...
int pathfind(vector<vector<bool> > const& blocked_grid, vector<Point> &path, bool &f, int x, int y){
  if (x==0 && y==0){
    f=true;
    path.push_back(Point(0,0));
    return 1;
  }
  if (x<0||y<0||blocked_grid[x][y]){
    return 0;
  }
  else
  {
    int a = pathfind(blocked_grid,path,f,x-1,y);
    if (f)
    {
      path.push_back(Point(x,y));
      return a;
    }
    int b = pathfind(blocked_grid,path,f,x,y-1);
    if (f)
    {
      path.push_back(Point(x,y));
    }
...
int main(int argc, char* argv[]) {
  ...
  vector<Point> path;
  print_grid(blocked_grid, path, start_x, start_y);

  // Start here with your code...
  bool f=false;
  int num_paths = pathfind(blocked_grid, path, f, start_x, start_y);
  cout << num_paths << endl;
